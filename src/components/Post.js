...
import { like, unlike } from "../redux/posts/postActions";
...
export default function Post(props) {
  ...
  const handleLike = () => {
    ...
    for (var i = 0; i < props.postLikes.length; i++) {
      if (props.postLikes[i]._id === auth.user._id) alreadyLiked = true;
    }
    if (alreadyLiked) dispatch(unlike(props.postId, auth.user._id));
    else if (auth.user._id) dispatch(like(props.postId, auth.user._id));
  };
    ...
    <ThemeProvider theme={theme}>
      ...
      >
        <CardHeader
          ...
          onClick={() => {
            navigate(`/user?id=${props.creator}`);
          }}
        />
